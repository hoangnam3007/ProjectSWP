...
public class forgetPassword extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean check = fpd.checkExistedRegister(email);
        if (!check) {
            String msg = "Email address does not exist";
            ...
            //if find user after check existed, assign a session with full infomation of user.
            rd = fpd.getUserByEmail(email);
            rd.setCode(code);
            boolean sendEmail = rs.checkEmailReset(rd);

            HttpSession session = request.getSession();
            session.setAttribute("getEmailFromForgetPass", rd);
            response.sendRedirect("verifiedForgetCode.jsp");
        }
