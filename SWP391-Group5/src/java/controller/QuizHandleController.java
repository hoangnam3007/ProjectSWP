...
public class QuizHandleController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (elapsedTime > duration) {
            ...
            autoSubmitQuiz(request, response, user, quizId);
            return;
        }

        // Regular form submission handling
        handleFormSubmission(request, response, user, quizId);
    }

    private void autoSubmitQuiz(HttpServletRequest request, HttpServletResponse response, User user, int quizId) throws IOException {
        HttpSession session = request.getSession();
        QuestionDAO questionDAO = new QuestionDAO();
        ...
        try {
            ...
            userQuizDAO.storeQuizResult(user.getUser_id(), quizId, score, isPass);
        } catch (SQLException ex) {
            Logger.getLogger(QuizHandleController.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void handleFormSubmission(HttpServletRequest request, HttpServletResponse response, User user, int quizId) throws IOException {
        HttpSession session = request.getSession();
        QuizDAO quizDAO = new QuizDAO();
        UserAttempQuizDAO userAttempQuizDAO = new UserAttempQuizDAO();
        QuestionDAO questionDAO = new QuestionDAO();
