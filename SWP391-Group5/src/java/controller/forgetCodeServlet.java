...
public class forgetCodeServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        RegisterDTO rd = fpd.getUserByEmail(email);
        String code = request.getParameter("code");

        if (!code.equals(codeFrom)) {
            String msg = "Code is incorrect. Check Email again";
            request.setAttribute("error_msg", msg);
            request.getRequestDispatcher("verifiedForgetCode.jsp").forward(request, response);
            return;
        } else {
            request.setAttribute("forgetUser", rd);
