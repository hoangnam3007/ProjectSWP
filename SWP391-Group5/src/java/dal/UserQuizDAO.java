...
public class UserQuizDAO extends DBContext {
    ...
    public List<UserDoneQuiz> getAllQuizDoneByUser(int userId) {
        ...
        try {
            ...
            query.append("""
                         ...
                         on q.quiz_id = uq.quiz_id where user_id = ?
                         """);
            list.add(userId);
    ...
    public void storeQuizResult(int userId, int quizId, int score, boolean isPass) throws SQLException {
        String query = """
            IF EXISTS (SELECT 1 FROM user_has_done_quiz WHERE user_id = ? AND quiz_id = ?)
                UPDATE user_has_done_quiz SET score = ?, is_pass = ? WHERE user_id = ? AND quiz_id = ?
            ELSE
                INSERT INTO user_has_done_quiz (user_id, quiz_id, score, is_pass) VALUES (?, ?, ?, ?)
        """;
        try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
            preparedStatement.setInt(1, userId);
            preparedStatement.setInt(2, quizId);
            ...
            preparedStatement.setBoolean(10, isPass);
            preparedStatement.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
            throw e;
    ...
    public static void main(String[] args) {
        UserQuizDAO o = new UserQuizDAO();
        List<UserDoneQuiz> list = o.getAllQuizDoneByUser(1);
